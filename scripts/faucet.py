...
class MockUSDCFaucet:
    ...
    def send_tokens(self, to_address, amount=100):
        ...
        if receipt.status == 1:
            ...
            print(f"✅ Confirmado! Nuevo balance: {new_balance} USDC")
        else:
    ...
    def faucet_tokens(self, to_address, amount=10000):
        ...
        if amount_wei > max_faucet:
            ...
            print(f"⚠️ Ajustando cantidad a máximo permitido...")
...
if __name__ == "__main__":
    ...
    FAUCET_API_ADDRESS = "0xEe4A90a9d3E24cBB7b1522552717D4F379d405B2"
    is_owner = faucet.check_owner()
    if not is_owner:
        print("❌ ERROR: No eres el owner del contrato")
        exit()

    print(f"💰 Balance del faucet API: {faucet.get_balance(FAUCET_API_ADDRESS):,.2f} USDC\n")

    AMOUNT = 100_000_000
    
    print(f"🎯 Minteando {AMOUNT:,.0f} USDC al faucet API...")
